/*
 * created by $
 */


package com.tabakshop.data;

import com.tabakshop.utils.TempEmailService;

public class Data {
    public static final String EMAIL_URL = "https://www.1secmail.com/";
    public static final String ACTIVATE_MESSAGE = "Аккаунт, успешно, активирован";
    public static final String REGISTRATION_MESSAGE = "Registration";
    public static final String PASSWORD = "Password1234!";
    public static final String CHECKBOX_MESSAGE = "Klicke dieses Kästchen an, wenn du fortfahren möchtest.";
    public static final String DO_NOT_MATCH_MESSAGE = "Passwords do not match";
    public static final String EMAIL_EXAMPLE = "test34@gmail.com";
    public static final String PASSWOR_EXAMPLE = "7465HHHY***$64";
    public static final String EMAIL_EXAMPLEUNIQUSER = "0t49ttrk3g@txcct.com";
    public static final String PASSWOR_EXAMPLEUNIQUSER = "7465HHHffY***$64";
    public static final String EMAIL_ADMIN = "tobeornottobe@gmail.com";
    public static final String PASSWOR_ADMIN = "myadminpasswordIsStrong***33";
    public static final String EXISTS_USER_MESSAGE = "User already exists. Please try a different email.";
    public static final String NOT_VALID_PASSWORD_MESSAGE = "Password must be in a valid format.";
    public static final String EMPTY = "";

    public static final String LOGIN_MESSAGE = "Login";
    public static final String LOGOUT_BUTTON  = "Sign out";
    public static final String SIGNOUT_BUTTON  = "Sign out";
    public static final String SIGNIN_LINK  = "Sign in";
    public static final String ADMIN_GREETING  = "Hello, Admin :)";

    public static final String NOT_EXIST_PASSWORD = "7465HffHHY***$64";
    public static final String WRONG_EMAIL_EXAMPLE = "test3@gmail.com";
    public static final String CATALOG_INSCRIPTION = "Product Catalog";
    public static final String ARCHIVE_INSCRIPTION = "Archived Products";
    public static final String ORDERS_INSCRIPTION = "View orders";
    public static final String EXAMPLE_PRODUCT = "Dunhill";
    public static final String EXAMPLE_PRODUCT_NEGATIVE = "123";
    public static final String EXAMPLE_PRODUCT_PARTIAL = "L";
    public static final String NAME_NEW_PRODUCT = "Кубинские Сигары";
    public static final String SUCCESS_MESSAGE_ADD_TO_CART = "Product added to cart!";
    public static final String NEW_DESCRIPTION = "Вкус победы";
    public static final String NEW_CHARACTERISTICS = "Сделано в лучших подвалах города";
    public static final Double NEW_PRICE = 13.66;
    public static final int NEW_QUANTITY = 200;
    public static final String MESSAGEUNREGISTERUSER = "Please register or log in to your account to add items to your cart.";

    public static final String tempEmail = TempEmailService.generateTempEmail();


}
